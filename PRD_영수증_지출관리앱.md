...
- [x] `pdf2image`로 샘플 PDF 변환이 로컬에서 성공한다
- [x] Vercel 서버리스에서 `/tmp` 경로 사용 가능 여부를 공식 문서로 확인한다
...
> **Phase 0 검증 결과 반영**: langchain 0.2.0 → 1.2.18, langchain-upstage 0.1.0 → 0.7.7로 업데이트.
> `document-digitization-vision` 모델은 chat completions API에서 미지원 — OCR은 `UpstageDocumentParseLoader`로, JSON 구조화는 `ChatUpstage(model='solar-pro')`로 분리한다.
